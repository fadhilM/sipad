...
<section class="content-header">
    ...
    <ol class="breadcrumb">
        ...
        <li><a href="{{route('suratMasuk.index')}}"><i class="fa fa-dashboard"></i>Index Surat Masuk</a></li>
...
<div class="box box-primary">
    ...
    <!-- /.box-header -->
    <!-- form start -->
    <form role="form" action="{{route('suratMasuk.update',$surat_masuk->id_surat)}}" method="POST" enctype="multipart/form-data">
        @method('PUT')
        @csrf
        <div class="box-body">
            <div class="form-group @error('no_surat') has-error @enderror">
                ...
                <input name="no_surat" type="text" class="form-control" id="no_surat" placeholder="Masukkan Nomor Surat" value="{{$surat_masuk->no_surat}}">
                @error('no_surat')
            ...
            <div class="form-group @error('tujuan') has-error @enderror">
                ...
                <input name="tujuan" type="text" class="form-control" id="tujuan" placeholder="Masukkan Tujuan Surat" value="{{$surat_masuk->tujuan}}">
                @error('tujuan')
                <span class="help-block">{{$message}}</span>
                @enderror
            </div>
            <div class="form-group @error('dari') has-error @enderror">
                ...
                <input name="dari" type="text" class="form-control" id="dari" placeholder="Masukkan Pengirim Surat" value="{{$surat_masuk->dari}}">
                @error('dari')
                <span class="help-block">{{$message}}</span>
                @enderror
            </div>
            <div class="form-group @error('untuk') has-error @enderror">
                ...
                <input name="untuk" type="text" class="form-control" id="untuk" placeholder="Masukkan Penerima Surat" value="{{$surat_masuk->untuk}}">
                @error('untuk')
                <span class="help-block">{{$message}}</span>
                @enderror
            </div>
            <div class="form-group @error('id_category') has-error @enderror">
                ...
                <select name="id_category" class="form-control select2" style="width: 100%;" id='id_category'>
                    @foreach ($categories as $category)
                        @if ($category->id_category==$surat_masuk->id_category)
            ...
            <div class="form-group @error('keterangan') has-error @enderror">
                ...
                <textarea name="keterangan" id="keterangan" class="form-control" rows="3" placeholder="Masukkan Keterangan">{{$surat_masuk->keterangan}}</textarea>
                @error('keterangan')
                <span class="help-block">{{$message}}</span>
                @enderror
            </div>
            <div class="form-group @error('file_surat_masuk') has-error @enderror">
                <label for="file_surat_masuk" @error('file_surat_masuk') class="control-label" @enderror>File Surat Masuk</label>
                <div class="input-group margin">
                    <span class="input-group-btn">
                        <a href="{{route('suratMasuk.download',$surat_masuk->id_surat)}}" class="btn btn-info btn-flat">Download File</a>
                    </span>
                    <label type="text" class="form-control">{{$surat_masuk->filename}}</label>
                </div><br>
                <input type="file" id="file_surat_masuk" name="file_surat_masuk">
                <input type="hidden" name="dir" value="{{$surat_masuk->dir}}">
